...
\begin{itemize}
    \item \textbf{Git} pour le versionnement du projet
    \item \textbf{Discord} pour la communication entre les membres du groupe et avec le tuteur
    \item \textbf{réunions régulières} (toutes les 1 à 2 semaines) pour faire le point sur l'avancement
    \item Communication \textbf{quotidienne} entre les membres pour coordonner le travail
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Prototype initial client--serveur
    \end{itemize}
    \vspace{0.5cm}
...
\begin{frame}[fragile]{Architecture}
    ...
    \textbf{Communication avec socket}
        \begin{itemize}
            ...
            \item Ip et port précisé en ligne de commande
        \end{itemize}

    \textbf{Classes Spécialisées}
        \begin{itemize}
            \item ServerNetwork pour la gestion des nouveaux client et des packets
            \item ClientNetwork pour la gestions des packets
        \end{itemize}

    \textbf{Multi-Threading}
        \begin{itemize}
            \item Fonctions de ServerNetwork et ClientNetwork exécuté dans un thread
            \item Dépilement des packets dans la boucle principale
        \end{itemize}
...
\begin{itemize}
    \item Utilisé à la fois par le client et le serveur
    \item Peut être généré dynamiquement par un objet du serveur
\end{itemize}
\vspace{0.4cm}
...
\begin{itemize}
    \item Utilisé pour les échanges réseaux
    \item Un champ \textit{gf::id} pour les identifier lors de la désérialisation
    
    \item Nomenclature spécifique :
    \begin{itemize}
            \item{ServerXXX = envoyé par le server}
            \item{ClientXXX = envoyé par le client}
    \end{itemize}
\end{itemize}
...
\begin{frame}[fragile]{Sérialisation/Déserialisation}
    ...
    \textbf{Opérateur}
    \begin{itemize}
        \item Operateur | avec un type Archive
    \end{itemize}

    \textbf{Sérialisation/Envoi}
    \begin{itemize}
        \item Fonction \textit{gf::Packet::is(obj)} pour sérialiser en suite d'octet
        \item Fonction \textit{gf::TcpSocket::sendPacket(packet)} pour envoyer le packet
    \end{itemize}
    \textbf{Déserialisation}
    \begin{itemize}
        \item Switch case pour identifier le type
        \item Fonction \textit{gf::Packet::as(objType)} pour désérialiser
    \end{itemize} 
