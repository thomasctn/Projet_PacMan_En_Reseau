...
\end{lstlisting}
\vspace{0.5cm}
Ces deux types de structures ont un \textbf{opérateur |} avec un type \textit{Archive}, afin de pouvoir être sérialisé et désérialisé automatiquement
\vspace{0.5cm}
\begin{lstlisting}
...
La sérialisation et désérialisation des structures de communication sont faite grâce aux classes \texttt{gf::Serializer}, \texttt{gf::Deserializer} et \texttt{gf::Packet}, via la fonction \texttt{gf::Packet::as} , et la fonction \texttt{gf::Packet::is}, fournies par \textit{GamedevFramework}

\subsection{Protocole}
Lors de son exécution, le client cherche à se connecter au serveur à l'adresse IP et au port passé en paramètre. Le serveur, s'il existe, reçoit cette demande, et associe au client un identifiant, sous la forme d'un entier positif, avant de répondre au client avec cette identifiant et la liste des salons.
Grâce à cette identifiant, le client peut savoir qui il est parmi la liste des jouers.
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    ...
    \caption{Version simplifié de la connexion}
\end{figure}
