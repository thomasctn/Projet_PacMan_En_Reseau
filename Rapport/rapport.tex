...
\subsection{Rooms : gestion des parties}
...
Dans le cadre d’un Pac-Man multijoueur, le client doit gérer des contraintes spécifiques : latence réseau, cohérence des états, fluidité visuelle, transitions entre différentes phases de jeu et gestion d’une interface utilisateur complète (menu, lobby, partie, fin de jeu).
...
Elle ne possède pas la responsabilité de déterminer ou est un joueur, ou quels sont les paramètres actuels d'un lobby.
...
Dans le cas du lobby, le fonctionnement est différent.
...
À l’inverse, certaines actions issues des entrées utilisateur sont envoyées au serveur, comme les déplacements en jeu ou les interactions dans le lobby (changer un paramètre, se déclarer prêt, etc.).
...
Par exemple, dans le lobby, si un joueur est prêt, l’entité affiche “(prêt)”, sinon “(pas prêt)”.
...
\begin{figure}[H]
    ...
    \caption{Échanges entre le client et le serveur lors de la connexion/lobby}
\end{figure}
