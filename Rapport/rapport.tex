...
Une fois les opérateurs implémentés, les structures peuvent être empaqueté via la fonction \texttt{gf::Packet::is(\textit{data})} et dépaqueté avec la fonction \texttt{gf::Packet::as<\textit{typename}>()}
...
Une fois la réponse du serveur reçu, le client sauvegarde son identifiant, afin de savoir qui il est parmi la liste des jouers, puis change de scène, et liste les salons qu'il l'a reçu.\\
\paragraph{Liste des salons}
Dans la liste des salons, le joueur peut choisir de rejoindre un salon existant, ou un en crée un nouveau. Lorsque l'une des deux options est choisi, le client envoi une demande de création ou de jointe d'un salon au serveur. Le serveur traite cette demande, en ajoutant le joueur dans le salon créé/rejoint, et envoi au client la liste des joueurs et les paramètres du salon. Dans le cas où le joueur rejoind un salon, le serveur envoi aussi à tous les autres clients du salon la nouvelle liste des joueurs pour les tenir à jour. Si le client tente de rejoindre un salon qui n'existe pas ou qui est plein le serveur en crée un nouveau.
Une fois la réponse du serveur reçu, le client change de scène (vers la scène du salon), et affiche les informations reçues.
\begin{figure}[H]
    \centering
...
Pour chaque tentative d'action (se déplacer, rejoindre une Room, etc...), le client envoi au serveur une requête, sous la forme d'une structure de "communication", contenant les information de l'action à effectuer. Le serveur, de son coté, reçoit cette requête, et l'empile dans la file, pour être traité avec les requêtes des autres clients dans la boucle de jeu.
Pour chaque packet dépilé, le serveur cherche à l'identifier dans un switch case, en regardant si le type du packet correspond à celui d'une des structures de "communication" pouvant être envoyé par le client, avant de le désérialisé, et traiter les données.\\

Si la requête du client nécessite une mise à jour des autres client (Par exemple, le déplacement d'un joueur), le serveur \textit{broadcast}, pour informer tous les clients.\\

De plus, dans la majorité des cas, le serveur ne transmet pas aux clients la modification lié à l'action d'un client, mais l'entièreté des données. Par exemple, lorsqu'un joueur rejoind une partie ou change de rôle, le serveur envoi l'intégralité des données des joueurs de la partie à chaque client, au lieu de juste envoyer les données du joueur en question.
Cette approche est privilégié à celle de la transmission de chaque modification pour la simplicité et la faible quantité de données à transmettre.
