...
\subsection{Structure du plateau}

Le plateau est modélisé sous forme d’une grille en deux dimensions, composée de deux type de cases, les cases que l'on peut traversée comme le sol et les cases qu'on ne peut pas traverser comme les murs. 
...
\begin{itemize}
  \item Le \textbf{TCP} avec \textit{gf::TcpSocket} et \textit{gf::TcpListener}
  \item Le \textbf{UDP} avec \textit{gf::UdpSocket}\\
\end{itemize}

Pour ce projet, le protocol \textbf{TCP} est choisi, car la vitesse qu'offre UDP n'est pas nécessaire pour un tel projet.

Le serveur contient une socket de connexion \textbf{TCP} (\textit{gf::TcpListener}), initialisé en même temps que lui. Cette socket gère les connexions des clients dans un thread, en assosiant une socket de communication pour chaque client. Le client, de son coté, initialise directement une socket de communication, utilisée dans la boucle principale.
...
\item Les structure \textit{communes} sont des structures contenant des données utilisé à la fois par le client et le serveur. Parmi eux, il y a, par exemple la structure PlayerData, contenant les informations relatives à un joueur (position, points de vie, etc...).
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/lc_pd.png}
    \caption{Structure \textit{commune} \textit{PlayerData}}
\end{figure}


\item Les strucutes de \textit{communication} sont les structures sérialisé pour être envoyé, et reçu avant d'être désérialisé avec pour être utilisé. Ces structures contiennent la plupart du temps une ou plusieurs structures \textit{communes}, mais ils ont aussi un champs \textit{type} du type \textit{gf::Id}, afin d'être indentifié lors de la déserialisation.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/lc_slrp.png}
    \caption{Structure de \textit{communication} \textit{ServerListRoomPlayers}}
\end{figure}
\end{itemize}

Ces deux types de structures ont un opérateur | avec un type \textit{Archive} implémenté, afin de pouvoir être sérialisé et désérialisé automatiquement
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/lc_o_pd.png}
    \caption{L'opérateur | pour la structure \textit{PlayerData}}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/lc_o_slpr.png}
    \caption{L'opérateur | pour la structure \textit{ServerListRoomPlayers}}
\end{figure}

\paragraph{Sérialisation et désérialisation}

La sérialisation et désérialisation des structures de communication sont faite graces au classe \textit{gf::Serializer}, \textit{gf::Deserializer} et \textit{gf::Packet}, via la fonction \textit{gf::Packet::as} , et la fonction \textit{gf::Packet::is}, fournies par \textit{GamedevFramework}
...
Si la requête du client nécessite une mise à jour des autres client (Par exemple, le déplacement d'un joueur), le serveur broadcast, pour informer tous les clients.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/ds_client.png}
    \caption{Version simplifié de la ligne de vie du client}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/ds_client.png}
    \caption{Version simplifié de la ligne de vie du serveur}
\end{figure}
