...
Une seconde phase plus intensive, organisée en trois sprints d’une semaine durant le mois de janvier, a permis de transformer ce prototype en une version fonctionnelle et stable du jeu.
...
Le projet a été en suivant une organisation inspirée des méthodes Agile, avec une progression de type incrémentale afin de valider régulièrement les choix techniques et d’adapter le développement aux difficultés rencontrées. Plutôt que de se baser sur un cahier des charges figé, bien qu'il soit défini, l’équipe a avancé par itérations successives, chaque sprint visant un objectif fonctionnel précis et donnant lieu à une version exécutable, même partielle. Cette approche a réduit les risques liés à l’architecture réseau et évité l’accumulation d’erreurs détectées trop tardivement, tout en s’inspirant du cadre Scrum qui est adapté au contexte universitaire et qui repose sur des cycles courts et une communication continue.

L’équipe était composée de trois étudiants, les tâches ont été répartit  comme il suit :\\
\begin{itemize}
  \item Thomas s’est chargé principalement du développement serveur, incluant la gestion des connexions, la logique centrale, la génération procédurale et les intelligences artificielles.
  \item Auriane a développé le client, gérant l’interface graphique, l’affichage et les interactions utilisateur.
  \item Benoît est intervenu à la fois sur client et serveur, avec un focus sur la communication réseau et la cohérence des échanges.\\
\end{itemize}

Les décisions techniques majeures étaient prises de manière collaborative pour assurer la cohérence globale du projet.
...
L’introduction du lobby a nécessité une réorganisation des états serveur (connecté, lobby, en partie) et l’isolation complète des instances de jeu, chaque room disposant de sa propre boucle et de son propre état. La boucle serveur, le c\oe ur de l’architecture autoritaire, centralisait les décisions logiques et la synchronisation. Malgré des difficultés liées aux transitions d’états et à la synchronisation lors de créations simultanées de rooms, les itérations successives ont permis de stabiliser l’ensemble. À la fin du sprint, l’architecture était modulaire, le lobby fonctionnel et les bases techniques solidement établies pour la suite du développement.

En parallèle des aspects techniques, ce premier sprint a été fortement guidé par les réunions de projet hebdomadaires avec notre tuteur de projet ainsi que des communications via Discord. Ces échanges réguliers ont permis de cadrer les premières décisions d’architecture et d’éviter de partir dans de mauvaises directions dès le départ. Le fait de commencer par une base simple puis de la faire évoluer progressivement correspondait bien à une approche Agile, avec des itérations courtes et validées régulièrement. Les retours du tuteur ont notamment aidé à prioriser les éléments essentiels et à structurer proprement le serveur dès les premières étapes.

\subsection{Sprint 2 : Développement du client et intégration des bots}
...
Étant donné que nous étions encore en janvier au début de ce sprint, les réunions de projet hebdomadaires de ce mois ont également joué un rôle important durant ce dernier. Le passage d’une première version fonctionnelle à une version plus structurée du client (notamment avec l’introduction du gestionnaire de scènes) a été encouragé et expliqué par le tuteur. Ce changement de conception, passant d’un système assez basique à une organisation plus modulaire, illustre bien une évolution Agile du projet. Ces réunions ont aussi permis de mieux comprendre certains aspects plus complexes d'un jeu en réseau et de les implémenter plus efficacement grâce aux explications et retours réguliers.

\subsection{Sprint 3 : Stabilisation, amélioration visuelle et débogage intensif}
...
Durant ce dernier sprint les réunions avec le tuteur se sont aussi concentrées sur le suivi du rapport de projet : le plan a pu être présenté au tuteur, puis ajusté selon ses retours, tout comme certains éléments de la soutenance (notamment lors d’une soutenance blanche). Cela a permis d’avancer de manière plus sereine sur la finalisation du projet, aussi bien sur le plan technique que sur la présentation de ce dernier.

Globalement, les réunions de projet et les comptes-rendus associés ont été un élément clé du bon déroulement du projet. Elles ont permis de garder une progression régulière, de corriger rapidement les erreurs et de mieux comprendre les choix techniques importants. Cette organisation s’inscrit pleinement dans une démarche Agile, avec un suivi continu et des ajustements progressifs tout au long du développement pour diriger un projet simple et basique au début vers un projet complètement developpé et terminé à la fin.

\section{Gestion des tests}

Pour ce projet, il a été choisi de ne pas implémenter de tests unitaires, car l'architecture choisie pour le client et le serveur n'est pas idéale pour la mise en place de tests sur un composant précis : beaucoup de composants dépendent les uns des autres, et donc tester le bon fonctionnement d'un composant revient, dans la plupart des cas, à tester le bon fonctionnement d'un autre composant, ce qui demanderait un temps considérable, jugé trop précieux pour cela.

Le bon fonctionnement du code produit durant le projet a donc été assuré par des tests fonctionnels : à chaque modification du code d'un composant, l'entièreté du code du projet était recompilé, puis exécuté, afin de voir si le logiciel résultant fonctionne conformément aux exigences définies sans avoir de comportements inattendus.

Ces tests fonctionnels ont permis une progression rapide et efficace du projet, en identifiant rapidement les comportements inattendus du logiciel, puis en isolant et corrigeant la ou les portions de code à l'origine des problèmes.
...
Pour finir, ce projet illustre l’importance d’une architecture pensée, d’une organisation clair et d’une grande capacité d’adaptation dans la réussite d’un développement logiciel.
