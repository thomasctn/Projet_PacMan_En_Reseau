...
Avant de développer le jeu complet, un prototype minimal client-serveur a été conçu afin de valider les bases techniques du projet.

L’objectif de ce prototype était de :

\begin{itemize}
    \item Vérifier le bon fonctionnement de la communication réseau.
    \item Mettre en place une architecture client-serveur autoritaire.
    \item Tester la gestion des threads côté serveur.
    \item Établir une base de synchronisation entre plusieurs clients.
\end{itemize}

Ce prototype consistait en une application simple où un client contrôlait le déplacement d’un cube sur un plan 2D. 

Le fonctionnement était le suivant :

\begin{enumerate}
    \item Le client envoie son intention de déplacement au serveur.
    \item Le serveur reçoit cette intention.
    \item Le serveur valide la demande.
    \item Le serveur met à jour la position.
    \item Le serveur diffuse (broadcast) la nouvelle position à tous les clients.
\end{enumerate}

Cette approche permet de poser les bases d’un \textbf{serveur autoritaire}, garantissant que :
\begin{itemize}
    \item Les clients ne peuvent pas modifier directement l’état du jeu.
    \item Toutes les décisions critiques sont centralisées.
    \item La cohérence du jeu est assurée.
\end{itemize}

Ce prototype a servi de fondation pour la suite du développement, notamment pour la gestion des rooms, la synchronisation des joueurs et l’implémentation de la logique de jeu complète.
...
Afin de gérer plusieurs connexions simultanées, le serveur repose sur une architecture multithread.

Chaque client connecté dispose :
\begin{itemize}
    \item d’un thread dédié à la réception des paquets,
    \item d’une file d’attente (queue) pour stocker les messages entrants,
    \item d’un traitement synchronisé côté logique serveur.
\end{itemize}

Cette organisation permet :

\begin{itemize}
    \item d’éviter le blocage du serveur lors d’attentes réseau,
    \item de séparer la communication de la logique métier,
    \item de garantir une meilleure réactivité globale.
\end{itemize}

Le serveur conserve toutefois une logique autoritaire : toutes les décisions impactant l’état du jeu sont prises centralement.

\subsubsection*{Transformation du prototype}

Le prototype initial a servi de base structurelle. Cependant, plusieurs adaptations ont été nécessaires :

\begin{itemize}
    \item Séparation claire entre la gestion réseau et la logique de jeu.
    \item Introduction d’un système de \textit{Lobby}.
    \item Ajout d’un système de \textit{Room}.
    \item Encapsulation de la logique de partie dans une classe dédiée \textit{Game}.
\end{itemize}

L’architecture finale suit une chaîne de responsabilité :
...
\begin{itemize}
    \item \textbf{Server} : gestion des connexions et routage des paquets.
    \item \textbf{Lobby} : gestion des salons et création des parties.
    \item \textbf{Room} : gestion des joueurs et préparation de la partie.
    \item \textbf{Game} : logique complète du jeu.
\end{itemize}

Cette organisation améliore la lisibilité du code et permet d’isoler les responsabilités.

\subsubsection*{Première implémentation du plateau}

La première version du plateau utilisait une grille simple composée de cases murales et traversables.

L’objectif initial était de :

\begin{itemize}
    \item Vérifier les collisions.
    \item Tester les déplacements synchronisés.
    \item Préparer l’intégration future d’un générateur de labyrinthe.
\end{itemize}

Cette étape intermédiaire a permis de valider la cohérence entre :
\begin{itemize}
    \item La représentation logique du plateau côté serveur.
    \item L’affichage côté client.
\end{itemize}

Le plateau est progressivement devenu une structure centrale du projet, servant de base à la génération procédurale et à l’intelligence artificielle des fantômes.
